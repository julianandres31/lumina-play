...
const AudiovisualContent = () => {
    ...
    const [newCastMember, setNewCastMember] = useState({ actorId: "", character: "", actorType: "Secondary" });
    ...
    const fetchData = async () => {
        try {
            ...
            setContents(contentRes.data.sort((a: AudiovisualContent, b: AudiovisualContent) => a.id - b.id));
    ...
    const fetchCast = async (contentId: number) => {
        try {
            ...
            const contentCast = response.data.filter((item: any) => item.audiovisualContentId === contentId || item.audiovisualContent.id === contentId);
            setCast(contentCast);
        } catch (error) {
    ...
    const handleDeleteCastMember = async (actorId: number) => {
        ...
        try {
            await api.delete(`/api/actor-audiovisual/${actorId}/${selectedContentId}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (formData.selectedGenreIds.length === 0) {
            toast({ variant: "destructive", title: "Debes seleccionar al menos un género" });
            return;
        }
        ...
        <Layout>
            <div className="container mx-auto px-4 py-8 lg:px-8 mt-20">
                ...
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-bold text-foreground">Contenido Audiovisual</h1>
                    <Dialog open={isDialogOpen} onOpenChange={(open) => {
                        ...
                        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                                <DialogTitle>{editingId ? "Editar" : "Crear"} Contenido Audiovisual</DialogTitle>
                            </DialogHeader>
                            <form onSubmit={handleSubmit} className="space-y-6 mt-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-1 block">Fecha de Estreno</label>
                                        <Input
                                            type="date"
                                            value={formData.relaseDate}
                                            onChange={(e) => setFormData({ ...formData, relaseDate: e.target.value })}
                                            required
                                        />
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-1 block">Clasificación de Edad</label>
                                        <Input
                                            type="number"
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-1 block">País de Producción</label>
                                        <Input
                                    ...
                                    <div>
                                        <label className="text-sm font-medium mb-1 block">Director</label>
                                        <select
                                ...
                                <div>
                                    <label className="text-sm font-medium mb-1 block">Trailer URL</label>
                                    <Input
                                ...
                                <Button type="submit" className="w-full">Guardar Contenido</Button>
                            </form>
                        </DialogContent>
                    </Dialog>
                    ...
                    <Dialog open={isCastDialogOpen} onOpenChange={setIsCastDialogOpen}>
                        <DialogContent className="max-w-2xl">
                            ...
                            {}
                            <form onSubmit={handleAddCastMember} className="flex gap-2 items-end mb-6 p-4 bg-muted/20 rounded-lg">
                                <div className="flex-1">
                                    <label className="text-xs font-medium mb-1 block">Actor</label>
                                    <select
                                        className="flex h-9 w-full rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm"
                                        value={newCastMember.actorId}
                                        onChange={(e) => setNewCastMember({ ...newCastMember, actorId: e.target.value })}
                                        required
                                    >
                                ...
                                <div className="flex-1">
                                    <label className="text-xs font-medium mb-1 block">Personaje</label>
                                    <Input
                                ...
                                <div className="w-32">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setNewCastMember({ ...newCastMember, actorType: e.target.value })}
                                    >
                            ...
                            <div className="border rounded-md overflow-hidden">
                                <Table>
                                    ...
                                    <TableBody>
                                        {cast.length === 0 ? (
                                            <TableRow>
                                                <TableCell colSpan={4} className="text-center py-4 text-muted-foreground">No hay actores en el reparto.</TableCell>
                                            </TableRow>
                                        ) : (
                                            cast.map((member: any) => (
                                                <TableRow key={member.id}>
                                                    <TableCell>{member.actor?.nameActor} {member.actor?.lastNameActor}</TableCell>
                                                    <TableCell>{member.character}</TableCell>
                                                    <TableCell>{member.actorType}</TableCell>
                                                    <TableCell className="text-right">
                                                        <Button variant="ghost" size="icon" onClick={() => handleDeleteCastMember(member.actorId || member.actor?.id)}>
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No hay contenido registrado.</TableCell>
                                </TableRow>
                            ) : (
                                contents.map((content) => (
                                    <TableRow key={content.id}>
                                        <TableCell>{content.id}</TableCell>
                                        <TableCell className="font-medium">{content.tittle}</TableCell>
                                        <TableCell>{content.relaseDate}</TableCell>
                                        <TableCell>
                                            {content.directorName || directors.find(d => d.id === content.directorId)?.nameDirector || content.directorId}
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell className="text-center">
                                            <Button variant="outline" size="sm" onClick={() => handleManageCast(content)}>
